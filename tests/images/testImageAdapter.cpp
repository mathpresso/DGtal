...
bool testSimple()
{
  ...
  typedef ImageAdapter<VImage, Z2i::Domain, functors::Identity, VImage::Value,
                       functors::Identity, functors::Identity>
  MyImageAdapter;
  ...
  trace.info() << "Restricted Image: " << restimage << "  "
  ...
  trace.warning() << "Original image at (3,3) = "
                  << (image)( Z2i::Point( 3, 3 ) ) << std::endl;
...
class MyTransValueFunctor
{
   ...
   */
  MyTransValueFunctor( const Value & aValue = 0 ) : myValue( aValue ){};
  ...
  inline Value operator()( const TInput & aInput ) const
  {
...
bool test_g_f_fm1()
{
  ...
  // ConstAdapter
  Z2i::Domain domain( Z2i::Point( 2, 2 ), Z2i::Point( 4, 4 ) );
  typedef ConstImageAdapter<VImage, Z2i::Domain, functors::Identity,
                            VImage::Value,
                            DGtal::functors::ConstValue<VImage::Value>>
  MyImageAdapter;
  ...
  typedef ImageAdapter<VImage, Z2i::Domain, functors::Identity, bool,
                       functors::Thresholder<VImage::Value>,
                       functors::Cast<VImage::Value>>
  MyImageAdapter2;
  ...
  trace.warning() << "Restricted image 2 at (2,2) = "
                  << (int)(restimage2)( Z2i::Point( 2, 2 ) ) << std::endl;
  trace.warning() << "Original image at (2,2) = "
                  << (image)( Z2i::Point( 2, 2 ) ) << std::endl;
  trace.warning() << "Original image at (3,3) = "
                  << (image)( Z2i::Point( 3, 3 ) ) << std::endl;

  restimage2.setValue( Z2i::Point( 2, 2 ), false );
  trace.warning() << "Restricted image with false at (2,2) = "
                  << (int)(restimage2)( Z2i::Point( 2, 2 ) ) << std::endl;
  nbok += ( restimage2( Z2i::Point( 2, 2 ) ) == true ) ? 1 : 0;
  nb++;
  trace.info() << "(" << nbok << "/" << nb << ") "
  ...
  trace.warning() << "Restricted image 2 at (2,2) = "
                  << (restimage2)( Z2i::Point( 2, 2 ) ) << std::endl;
  trace.warning() << "Original image at (2,2) = "
                  << (image)( Z2i::Point( 2, 2 ) ) << std::endl;

  typedef ImageAdapter<VImage, Z2i::Domain, MyTransValueFunctor<Z2i::Point>,
                       VImage::Value, functors::Identity, functors::Identity>
  MyImageAdapter3;
  ...
  trace.warning() << "Original image at (2,2) = "
                  << (image)( Z2i::Point( 2, 2 ) ) << std::endl;
  trace.warning() << "Original image at (4,4) = "
                  << (image)( Z2i::Point( 4, 4 ) ) << std::endl;
  trace.warning() << "Original image at (5,5) = "
                  << (image)( Z2i::Point( 5, 5 ) ) << std::endl;
...
bool testImageAdapter()
{
  ...
  Display2DFactory::drawImage<Gray>( aBoard, image, (unsigned char)0,
                                     (unsigned char)255 );
  ...
  trace.info() << "ImageAdapter: " << bell_tower << "  " << bell_tower.domain()
               << std::endl;
  ...
  aBoard.clear();
  Display2DFactory::drawImage<Gray>( aBoard, bell_tower, (unsigned char)0,
                                     (unsigned char)255 );
  ...
  trace.info() << "ImageAdapter: " << cars << "  " << cars.domain()
               << std::endl;
  ...
  aBoard.clear();
  Display2DFactory::drawImage<Gray>( aBoard, cars, (unsigned char)0,
                                     (unsigned char)255 );
  ...
  typedef ImageAdapter<VImage, DigitalSetDomain<Z2i::DigitalSet>,
                       functors::Identity, VImage::Value, functors::Identity,
                       functors::Identity>
  MyImageAdapter2;
  // BOOST_CONCEPT_ASSERT(( CImage< MyImageAdapter2 > )); // pb here

  functors::Identity idflD, idflV, idflVm1;
  MyImageAdapter2 floor_lamp( image, my_specific_domain_floor_lamp, idflD,
                              idflV, idflVm1 );
  //! [ImageAdapterWithSpecificDomain]

  trace.info() << "ImageAdapter: " << floor_lamp << "  " << floor_lamp.domain()
               << std::endl;
  ...
  aBoard.clear();
  Display2DFactory::drawImage<Gray>( aBoard, floor_lamp, (unsigned char)0,
                                     (unsigned char)255 );
...
int main( int argc, char ** argv )
{
  ...
  bool res = testSimple() && test_g_f_fm1() && test_range_constRange() &&
             testImageAdapter(); // && ... other tests
